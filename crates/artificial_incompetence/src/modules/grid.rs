...
impl GridZoneEncoder {
    ...
    pub fn forward(&self, batch_size: i64, device: Device) -> Tensor {
        ...
        let coord_e = self.coord_mlp.forward(&self.grid_spec.zone_features(device));
    ...
    pub fn mask(&self, batch_size: i64, device: Device) -> Tensor {
        Tensor::ones([batch_size, self.grid_spec.num_zones()], (Kind::Bool, device))
    }
}
