...
impl Coach {
    ...
    fn log_prob_of(&self, batch: &MultiBatch, action: &SampledRobotAction) -> Tensor {
        ...
        logp += categorical_log_prob(&out.zone_logits, &action.target_zone)
            * zone_like.to_kind(Kind::Float);

        logp += categorical_log_prob(&out.power_logits, &action.power_bin)
            * power_like.to_kind(Kind::Float);
