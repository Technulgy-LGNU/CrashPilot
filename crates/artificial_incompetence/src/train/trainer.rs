...
impl Trainer {
  pub fn new(vs: VarStore, num_worlds: usize) -> Self {
    Self::new_with_reward(vs, num_worlds, 1e-3, RewardMode::Full)
  }
  ...
  pub fn new_with_reward(
    ...
    let opt = tch::nn::Adam::default().build(&vs, learning_rate).unwrap();
  ...
  pub fn sync_states(&mut self, sim_states: Vec<WorldState>, states: Vec<GameState>) {
    assert_eq!(sim_states.len(), states.len());
    assert_eq!(states.len(), self.old_state.len());

    self.old_sim_state = sim_states;
    self.old_state = states;
    self.buf.clear();
    self.values.clear();
    self.sampled = SampledRobotAction::default();
    self.log_prob = Tensor::new();
  }
  ...
  pub fn step(&mut self, states: &[GameState], _dt: f32) {
    ...
    self.values = (0..states.len())
      .map(|i| value.double_value(&[i as i64, 0]))
      .collect::<Vec<_>>();
    self.sampled = sampled;
    self.log_prob = log_prob;
  }

  pub fn commands_for_world(&self, id: usize) -> Commands {
    self.data.get(id)
  }
  ...
  pub fn finish_step(&mut self, sim_states: Vec<WorldState>, states: Vec<GameState>) {
    ...
    for (id, state_sim) in sim_states.iter().enumerate() {
      ...
      rewards.push(compute_reward_for(
        self.reward_mode,
        old_sim,
        state_sim,
        old,
  ...
  pub fn finish_rollout(&mut self, states: &[GameState]) -> UpdateResult {
    ...
    let last_values = tch::no_grad(|| {
      ...
      (0..n)
        .map(|i| v.double_value(&[i as i64, 0]))
        .collect::<Vec<_>>()
    });
