...
pub fn build_action_masks(batch: &MultiBatch) -> Masks {
  ...
  let action_mask = Tensor::zeros(
    [b, MAX_ROBOTS_PER_TEAM, NUM_COMMANDS as i64],
    (Kind::Bool, device),
  );
  let teammate_mask = Tensor::zeros(
    [b, MAX_ROBOTS_PER_TEAM, MAX_ROBOTS_PER_TEAM],
    (Kind::Bool, device),
  );
  ...
  for i in 0..MAX_ROBOTS_PER_TEAM {
    ...
    let setup_mask = |cmd_type, val: &Tensor| {
      let val = val.view([b, 1, 1]).to_kind(Kind::Bool);
      action_mask
        .narrow(1, i, 1)
