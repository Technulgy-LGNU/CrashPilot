...
pub fn create_multicast_socket(multicast: Ipv4Addr, port: u16) -> anyhow::Result<UdpSocket> {
  ...
  match std_socket.set_nonblocking(true) {
    Ok(_) => (),
    Err(err) => return Err(anyhow::Error::msg("Error during setting socket to non blocking: ".to_owned() + &*err.to_string())),
  };

  match UdpSocket::from_std(std_socket) {
    Ok(socket) => Ok(socket),
    Err(err) => Err(anyhow::Error::msg("Error converting std socket to tokio socket: ".to_owned() +  &*err.to_string())),
  }
}
